...
with tab1:    
    ...
    with st.form("career_prediction_form"):
        ...
        for q in questions:
            if q["type"] == "slider":
                if q["column"] in ["GPA", "Extracurricular_Activities", "Field_Specific_Courses", "Projects"]:  # These use text inputs
                    input_value = st.text_input(
                        q["text"], 
                        value=str(q["min"]),  # Default value as string
                        key=f"text_input_{q['column']}"
                    )
                    # Convert to appropriate numeric type before saving to responses
                    try:
                        if q["round"] == "float":
                            numeric_value = float(input_value)
                        else:
                            numeric_value = int(float(input_value))  # Convert to float first, then int
                        st.session_state.responses[q["column"]] = numeric_value
                    except ValueError:
                        # If conversion fails, don't update the response yet
                        pass
                else:
